...
class CombinedBandMatcher:
    def __init__(self, simple_dir, full_dir, max_distance=0.01):
        ...
        # Get Fermi energy from DOSCAR in the full system directory.
    ...
    def get_fermi_level_from_doscar(self, directory):
        ...
        Assumes that the Fermi energy is the fourth number (index 3) on the sixth line.
        """
        doscar_path = os.path.join(directory, "DOSCAR")
        if not os.path.exists(doscar_path):
            print(f"DOSCAR not found in {directory}.")
            return None
    ...
    def parse_poscar_from_dir(self, directory):
        """
        Reads the POSCAR file in the given directory and returns a mapping:
            { atom_index: {"element": symbol, "coord": np.array([x, y, z])}, ... }.
        ...
        try:
            ...
            lattice_start = 2 if len(tokens) == 1 else 1
            ...
            if spec_line.startswith("selective"):
                ...
                pos_start = lattice_start+6 if ("direct" in spec_line or spec_line[0]=='d' or "cartesian" in spec_line or spec_line[0]=='c') else lattice_start+5
            coords = []
    ...
    def get_atom_mapping(self):
        ...
        For each simple atom, only full-system atoms with the same element are considered;
        the closest one (by Euclidean distance) is chosen if within max_distance.
        Stores the mapping as: { simple_atom_index: full_atom_index, ... }.
        """
    ...
    def load_combined_states(self):
        """
        Loads the PROCAR states via ProcarStatePlotter for the simple and full systems and combines
        the k-point data (weighted by k-point weights) for each unique band.
        The combined states are stored in self.simple_combined and self.full_combined.
        Also exports the combined composition matrices.
        """
    ...
    def assign_sequential_indices(self):
        """
        Sorts and assigns new sequential indices (stored in 'seq') to the combined states for both systems
    ...
    def adjust_simple_combined_band_energies(self):
        """
        Shifts the energies of the simple combined bands by a constant so that the lowest-energy simple band
        aligns with the lowest-energy full band.
    ...
    def shift_energies_to_fermi(self):
        ...
        so that the full system's Fermi level appears at 0 eV.
        """
        for state in self.simple_combined:
    ...
    def export_combined_bands(self, combined_states, out_filename):
        ...
        The states are assigned new sequential indices and sorted by energy.
        Each band header shows the new sequential index ("seq"), the original band number, and the shifted energy.
    ...
    def restrict_matrix(self, state, system="simple", flatten=True):
        """
        Restricts the combined state's orbital_matrix (of shape (N,4)) to only the rows corresponding to atoms present in the simple system.
        
        For system "simple": uses the simple system's POSCAR atom indices.
        For system "full": uses the atom mapping (for each simple atom, the corresponding full system row).
    ...
    def get_removed_matrix(self, state):
        ...
        where removed_matrix contains rows of the full system's orbital_matrix that do not correspond to any mapped simple atom.
        """
        if "orbital_matrix" not in state:
            return None
    ...
    def get_removed_orbital_info(self, state):
        """
        Computes a summary for the removed atoms from a full system state.
        For each removed atom, includes its full system atom number and orbital contributions (s, p, d, tot).
        The atoms are sorted in descending order by the 'tot' contribution, and the top three are returned
    ...
    def export_removed_bands(self):
        ...
        The format is identical to that of the combined composition matrix export.
        """
    ...
    def match_combined_bands(self):
        ...
        normalizes the resulting vector, and computes its inner product with every normalized simple system combined band's
    ...
    def total_composition(self, state, system):
        """
        Computes the total composition for a given combined state by summing the 'tot' column
        from the restricted composition matrix.
        """
        M = self.restrict_matrix(state, system=system, flatten=False)
        return np.sum(M[:, 3])

    def group_full_states_by_simple(self, matches):
        """
        Groups full system combined states by the sequential index of their matching simple state.
        Returns a dictionary: { simple_state_seq: [list of full states], ... }.
        """
        groups = defaultdict(list)
        for full_state, simple_state, overlap, ediff, rem in matches:
            if simple_state is not None:
                groups[simple_state["seq"]].append(full_state)
        return groups

    def final_plot(self, matches, energy_range=None):
        """
        Produces one overall figure with multiple subplots:
          - The top subplot displays all simple system combined bands as vertical delta lines (with inverted y-axis)
            and a dotted vertical black line at x=0.
          
          - For the full system bands, it creates one additional subplot per group (grouped by the matching simple state's
            sequential index) that have a matching simple state's energy within the given energy_range (if provided).
            Each group is plotted in its matching simple state color, and the x-axis of each group subplot is limited to
            the energy_range.
        
        If energy_range is None, all groups (with matching simple bands) are plotted.
        """
        # Group full states by matching simple state's sequential index.
        groups = self.group_full_states_by_simple(matches)
        # Get all simple states that have at least one matching full state.
        simple_all = [s for s in sorted(self.simple_combined, key=lambda s: s["energy"]) if s["seq"] in groups]
        # If energy range is provided, filter out simple states not in range.
        if energy_range is not None:
            emin, emax = energy_range
            simple_filtered = [s for s in simple_all if emin <= s["energy"] <= emax]
        else:
            simple_filtered = simple_all
        
        # Number of subplots: one for top (simple bands) + one for each filtered group.
        nrows = 1 + len(simple_filtered)
        fig, axes = plt.subplots(nrows=nrows, sharex=True, figsize=(10, 4 + 3*nrows))
        cmap = plt.get_cmap("tab10")
        simple_colors = { s["seq"]: cmap((s["seq"] - 1) % 10) for s in simple_filtered }
        
        # Top subplot: plot all simple combined bands (only those that are in the filtered list).
        ax_top = axes[0]
        for s in simple_filtered:
            energy = s["energy"]
            total = self.total_composition(s, "simple")
            ax_top.vlines(energy, 0, total, colors=simple_colors.get(s["seq"], "black"), lw=1)
        ax_top.axvline(x=0, color="black", linestyle="dotted")
        ax_top.set_ylabel("Total Composition")
        ax_top.set_title("Simple Combined Bands")
        ax_top.invert_yaxis()
        
        # For each filtered group, create its subplot.
        for i, s in enumerate(simple_filtered, start=1):
            ax = axes[i]
            group_full = groups[s["seq"]]
            for state in group_full:
                energy = state["energy"]
                total = self.total_composition(state, "full")
                ax.vlines(energy, 0, total, colors=simple_colors.get(s["seq"], "black"), lw=1)
            ax.axvline(x=0, color="black", linestyle="dotted")
            ax.set_ylabel("Total Composition")
            ax.set_title(f"Full Bands for Simple Band {s['seq']} (E={s['energy']:.4f} eV)")
            if energy_range is not None:
                ax.set_xlim(energy_range)
        axes[-1].set_xlabel("Energy (eV)")
        
        fig.suptitle("Combined Band Matching: Grouped Full System Bands", fontsize=16)
        plt.tight_layout(rect=[0, 0, 1, 0.95])
        out_fig = os.path.join(self.full_dir, "grouped_combined_band_matching_figure.png")
        plt.savefig(out_fig, dpi=300)
        plt.show()
        print(f"Grouped full system plots saved to {out_fig}")

    def export_matches(self, matches):
        """
        Exports the matching results to a tab-delimited text file in the full system directory.
        The output includes: FullBand(seq), FullEnergy, SimpleBand(seq), SimpleEnergy, Overlap,
        EnergyDifference, and RemovedOrbitals.
        """
        out_filename = os.path.join(self.full_dir, "combined_band_matching_results.txt")
        out_lines = []
        header = ("FullBand(seq)\tFullEnergy(eV)\tSimpleBand(seq)\tSimpleEnergy(eV)\t"
                  "Overlap\tEnergyDifference(eV)\tRemovedOrbitals")
        ...
        for full_state, simple_state, overlap, energy_diff, removed_info in matches:
            if simple_state is None:
                ...
                line = (f"{full_state.get('seq', full_state['band'])}\t{full_state['energy']:.4f}\t"
                        f"{simple_state.get('seq', simple_state['band'])}\t{simple_state['energy']:.4f}\t"
                        f"{overlap:.4f}\t{energy_diff:.4f}\t{removed_info}")
    ...
    def get_matching_energy_bounds(self, matches):
        """
        Returns a tuple (min_energy, max_energy) capturing the energy bounds of full system states that have a matching simple state.
        """
        energies = [full_state["energy"] for full_state, simple_state, overlap, ediff, rem in matches if simple_state is not None]
        if not energies:
            return None
        return (min(energies), max(energies))

    def run(self, energy_range=None):
        ...
        self.final_plot(matches, energy_range=energy_range)
        bounds = self.get_matching_energy_bounds(matches)
        if bounds is not None:
            print("Energy bounds for matching full system states:", bounds)
        else:
            print("No matching full system states found.")
        return matches
